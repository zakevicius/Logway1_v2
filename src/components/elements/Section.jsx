...
const Section = ({ t, data: { section, title, list } }) => {
  ...
  const checkInView = () => {
    if (
      window.innerHeight + window.pageYOffset - titleRef.current.offsetHeight >
      ulRef.current.offsetParent.offsetTop
    ) {
      if (shouldSlide) {
  ...
  return (
    <section className={section} id={section}>
      ...
      <div className='sectionContent'>
        <h1 ref={titleRef}>
          <span className='red'>{t(title)[0]}</span>
          {t(title).slice(1)}
        </h1>
